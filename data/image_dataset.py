...
class ImageDataset(data.Dataset, Configurable):
    ...
    def load_ann(self):
        ...
        for gt in self.gt_paths:
            lines = []
            reader = open(gt, 'r',encoding='latin-1').readlines()
            for line in reader:
                item = {}
                ...
                elif 'mavi' in self.data_dir[0]:
                    line = [i.strip('\ufeff').strip('\xef\xbb\xbf\x08') for i in parts]
                    if len(line) <= 8:
                        continue
                    poly = np.array(list(map(float, line[:8]))).reshape((-1, 2)).tolist()
                else:
